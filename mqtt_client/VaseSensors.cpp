...
VaseSensors::VaseSensors(uint8_t LIGHT_PIN, uint8_t SOIL_MOISTURE_PIN, unsigned int ENS160_I2C_ADDRESS, bool verbose ) {
...
int VaseSensors::getSoilMoisture() const{
    return analogRead(this->SOIL_MOISTURE_PIN);
}


int VaseSensors::getLightLevel() const{
    return analogRead(this->LIGHT_PIN);
}


int VaseSensors::getTemperature(){
...
void VaseSensors::setENS160DeepSleep() {
...
void VaseSensors::setENS160Reset() {
